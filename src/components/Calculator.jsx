...
const Calculator = () => {
  ...
  const handleBackspace = () => {
    if (equation.length > 0) {
      ...
      const lastNumber = newEquation.split(/[\s\(\)]/).filter(Boolean).pop() || '0';
  ...
  const calculate = () => {
    try {
      // Clean up the equation before calculating
  ...
  const getOperatorIcon = (op) => {
    switch(op) {
      case '+': return <Plus className="text-white" size={20} />;
      case '-': return <Minus className="text-white" size={20} />;
      case '×': return <X className="text-white" size={20} />;
      case '÷': return <Divide className="text-white" size={20} />;
      default: return op;
    }
  };
  ...
  const getBtnClassName = (btn) => {
    ...
    if (btn === <Delete size={20} />) return `${baseClass} bg-red-600 hover:bg-red-500`;
        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-4 gap-2 mt-4">
            {buttons.map((btn, index) => (
              ...
              >
                {typeof btn === 'string' && /[÷×\-+]/.test(btn) ? getOperatorIcon(btn) : btn}
              </button>
            ))}
          </div>
        </div>
